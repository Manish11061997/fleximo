...
function calculateMatchScore(job, resumeSkills, resumeTitles, resumeExperience, resumeKeywords) {
  ...
  if (allResumeKeywords.length > 0) {
    ...
    if (!hasKeywordInHeader && !hasKeywordInDesc && !hasAnySkillMatch) {
      return { score: 0, matchedSkills: [], matchedKeywords: [] };
    }
  }

  // ── 6. Minimum Relevance Gate ────────────────────────────────────────────
  const isDevTitle = /\b(developer|engineer|programmer|architect|data|devops|software|fullstack|frontend|backend|cloud|tech|consultant|specialist|lead|analyst|manager)\b/i.test(jobTitleLower);
  const isValidMatch = (
    (titleScore >= 30) ||
    (coreHeaderSkills.length >= 1) ||
    (hasKeywordInHeader) ||
    (hasKeywordInDesc) ||
    (coreDescSkills.length >= 1)
  );
  ...
  // Keyword match contribution (high priority for keyword searches)
  if (hasKeywordInHeader) {
    score += 35;
  ...
  if (titleScore >= 80 && (coreMatchedSkills.length >= 1 || hasKeywordInHeader)) {
    score = Math.max(score, 90);
  }
  if ((coreHeaderSkills.length >= 2 || hasKeywordInHeader) && (coreMatchedSkills.length >= 2 || hasKeywordInDesc)) {
