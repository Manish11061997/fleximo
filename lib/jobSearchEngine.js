...
function calculateMatchScore(job, resumeSkills, resumeTitles, resumeExperience, resumeKeywords) {
  ...
  // ── 0. Non-Technical Role Exclusion (Only if candidate did NOT search for it) ─────
  const hasExplicitKeywordMatch = allResumeKeywords.some(k => k && k.length >= 3 && jobTitleLower.includes(k.toLowerCase()));
  const hasExplicitTitleMatch = allResumeTitles.some(t => t && t.length >= 3 && jobTitleLower.includes(t.toLowerCase()));
  ...
  if (!hasExplicitKeywordMatch && !hasExplicitTitleMatch && isTechCandidate) {
