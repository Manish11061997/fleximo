...
const NON_TECH_TITLE_PATTERNS = [
  /\b(designer|graphic designer|ui\/ux|ui designer|ux designer|product designer|visual designer|motion designer|brand designer|design engineer|staff design engineer|illustrator|animator|3d artist|video editor)\b/i,
  ...
  /\b(hr generalist|recruiter|talent acquisition|people operations|administrative assistant|virtual assistant|executive assistant|office manager)\b/i,
  ...
  /\b(accounts receivable|accounts payable|accountant|bookkeeper|financial analyst|billing specialist|collections specialist|payroll)\b/i,
...
/**
 * Check if the job's title indicates a completely conflicting discipline from the user's search
 */
function hasDisciplineConflict(jobTitleLower, searchedKeywords = [], searchedTitles = [], searchedSkills = []) {
  const allSearch = [...searchedKeywords, ...searchedTitles, ...searchedSkills].map(s => String(s).toLowerCase());

  const isSearchingSalesforce = allSearch.some(s => /salesforce|apex|lwc|visualforce|sfdc/i.test(s));
  const isSearchingMobile     = allSearch.some(s => /flutter|dart|ios|swift|android|kotlin|react native|mobile/i.test(s));
  const isSearchingFrontend   = allSearch.some(s => /react|vue|angular|svelte|frontend|front-end|nextjs|css|html/i.test(s));
  const isSearchingData       = allSearch.some(s => /data scientist|machine learning|ml engineer|ai engineer|data science|nlp|computer vision|data analyst/i.test(s));
  const isSearchingDevOps     = allSearch.some(s => /devops|sre|site reliability|kubernetes|terraform|cloud infrastructure|platform engineer/i.test(s));
  const isSearchingBackend    = allSearch.some(s => /golang|go developer|django|fastapi|ruby on rails|laravel|spring boot|backend/i.test(s));

  if (isSearchingSalesforce) {
    // Salesforce queries MUST be Salesforce/CRM/Business Systems roles
    if (!/\b(salesforce|sfdc|crm|apex|lwc|visualforce|business systems|revenue cloud|marketing cloud)\b/i.test(jobTitleLower)) {
      return true;
    }
  }

  if (isSearchingMobile) {
    // Mobile searches must not match explicit backend, data, devops, ai, or golang titles
    if (/\b(golang|go developer|data engineer|data scientist|ai engineer|machine learning|ml engineer|devops|sre|security engineer|sales engineer|ruby on rails)\b/i.test(jobTitleLower) && !/\b(mobile|flutter|react native|ios|android)\b/i.test(jobTitleLower)) {
      return true;
    }
  }

  if (isSearchingFrontend) {
    // Frontend searches must not match explicit backend-only, data, ai, devops, or security titles
    if (/\b(golang|data engineer|data scientist|ai engineer|machine learning|ml engineer|devops|sre|security engineer|sales engineer|hardware|embedded|aem)\b/i.test(jobTitleLower) && !/\b(frontend|front-end|fullstack|full-stack|full stack|react|vue|angular|web)\b/i.test(jobTitleLower)) {
      return true;
    }
  }

  if (isSearchingData) {
    // Data Science searches must not match frontend, mobile, or sales roles
    if (/\b(frontend developer|front-end|ios developer|android developer|flutter developer|sales engineer|account executive)\b/i.test(jobTitleLower) && !/\b(data|machine learning|ai|ml|analytics|scientist)\b/i.test(jobTitleLower)) {
      return true;
    }
  }

  if (isSearchingDevOps) {
    // DevOps searches must not match frontend or mobile dev roles
    if (/\b(frontend developer|front-end|ios developer|android developer|flutter developer|ui\/ux|graphic)\b/i.test(jobTitleLower) && !/\b(devops|sre|cloud|infrastructure|platform|reliability|systems)\b/i.test(jobTitleLower)) {
      return true;
    }
  }

  if (isSearchingBackend) {
    // Backend searches must not match pure mobile or design roles
    if (/\b(ios developer|android developer|flutter developer|graphic designer|ui designer)\b/i.test(jobTitleLower) && !/\b(backend|software|developer|engineer)\b/i.test(jobTitleLower)) {
      return true;
    }
  }
...
function calculateMatchScore(job, resumeSkills, resumeTitles, resumeExperience, resumeKeywords) {
  const allResumeSkills   = (resumeSkills   || []).map(s => String(s).toLowerCase().trim()).filter(s => s.length >= 2);
  const allResumeTitles   = (resumeTitles   || []).map(t => String(t).toLowerCase().trim()).filter(t => t.length >= 2);
  const allResumeKeywords = (resumeKeywords || []).map(k => String(k).toLowerCase().trim()).filter(k => k.length >= 2);
  ...
  const jobHeaderLower = `${jobTitleLower} ${jobTagsLower}`;
  ...
  // ── 1. Discipline Conflict Gate ───────────────────────────────────────────
  if (hasDisciplineConflict(jobTitleLower, allResumeKeywords, allResumeTitles, allResumeSkills)) {
    return { score: 0, matchedSkills: [], matchedKeywords: [] };
  }
  ...
  for (const skill of allResumeSkills) {
    ...
    if (pattern.test(jobHeaderLower))    headerMatchedSkills.push(skill);
    else if (pattern.test(jobDescLower)) descMatchedSkills.push(skill);
  }
  const matchedSkills = Array.from(new Set([...headerMatchedSkills, ...descMatchedSkills]));

  // Core vs Generic Skill Breakdown
  const coreHeaderSkills = headerMatchedSkills.filter(s => !GENERIC_SKILLS.has(s.toLowerCase()));
  const coreDescSkills   = descMatchedSkills.filter(s => !GENERIC_SKILLS.has(s.toLowerCase()));
  const coreMatchedSkills = Array.from(new Set([...coreHeaderSkills, ...coreDescSkills]));
  ...
  for (const keyword of allResumeKeywords) {
    ...
    if (pattern.test(jobHeaderLower) || pattern.test(jobDescLower)) matchedKeywords.push(keyword);
  }

  // ── 5. Keyword-Specific Search Enforcement ──────────────────────────────
  let hasKeywordInHeader = false;
  let hasKeywordInDesc = false;

  if (allResumeKeywords.length > 0) {
    hasKeywordInHeader = allResumeKeywords.some(k => getBoundaryRegex(k).test(jobHeaderLower));
    hasKeywordInDesc   = allResumeKeywords.some(k => getBoundaryRegex(k).test(jobDescLower));
    const isDevTitle = /\b(developer|engineer|programmer|architect|lead|fullstack|frontend|backend|devops|cloud|data|scientist|analyst|consultant|specialist)\b/i.test(jobTitleLower);

    // Only strictly enforce keyword-in-header for pure keyword/direct searches (no resume skills)
    // For resume uploads with rich skills, we allow keyword-in-desc to count as a valid match
    const hasAnySkillMatch = matchedSkills.length > 0 || titleScore >= 25;
    if (!hasKeywordInHeader && !(hasKeywordInDesc && isDevTitle) && !hasAnySkillMatch) {
      return { score: 0, matchedSkills: [], matchedKeywords: [] };
    }
  }

  // ── 6. Minimum Relevance Gate ────────────────────────────────────────────
  const isDevTitle = /\b(developer|engineer|programmer|architect|data|devops|software|fullstack|frontend|backend)\b/i.test(jobTitleLower);
  const isValidMatch = (
    (titleScore >= 40) ||
    (coreHeaderSkills.length >= 1) ||
    (hasKeywordInHeader) ||
    (coreDescSkills.length >= 2 && isDevTitle) ||
    (hasKeywordInDesc && isDevTitle && (titleScore >= 20 || coreDescSkills.length >= 1))
  );

  if (!isValidMatch) return { score: 0, matchedSkills: [], matchedKeywords: [] };

  // ── 7. Score Calculation ─────────────────────────────────────────────────
  let score = 25; // Baseline for verified relevant match

  // Title contribution (up to 35 points)
  if      (titleScore >= 80) score += 35;
  else if (titleScore >= 50) score += 25;
  else if (titleScore >= 25) score += 15;

  // Core header skills (high visibility in title/tags) — up to 35 points
  if (coreHeaderSkills.length > 0) {
    score += Math.min(35, 20 + (coreHeaderSkills.length - 1) * 8);
  }

  // Core description skills — up to 25 points
  if (coreDescSkills.length > 0) {
    score += Math.min(25, 12 + (coreDescSkills.length - 1) * 5);
  }

  // Secondary/generic skills — up to 8 points
  const genericMatches = matchedSkills.filter(s => GENERIC_SKILLS.has(s.toLowerCase()));
  if (genericMatches.length > 0) {
    score += Math.min(8, genericMatches.length * 2);
  }

  // Custom keywords contribution — up to 15 points
  if (matchedKeywords.length > 0) {
    score += Math.min(15, 10 + (matchedKeywords.length - 1) * 3);
  }

  // Bonus for strong combined match
  if (titleScore >= 80 && coreMatchedSkills.length >= 1) {
    score = Math.max(score, 90);
  }
  if (coreHeaderSkills.length >= 2 && coreMatchedSkills.length >= 3) {
...
async function fetchRemoteOKJobs() {
  try {
    ...
    return jobs
      .filter(job => job.id && job.position) // Filter valid entries
      .map(job => ({
        id: `remoteok-${job.id}`,
        title: job.position || '',
        company: job.company || '',
...
async function fetchHimalayasJobs(query = '') {
  ...
  try {
    if (query) {
      ...
      if (response.ok) {
        ...
        for (const job of jobs) {
          allJobs.push({
            id: `himalayas-${job.id || job.slug || allJobs.length}`,
            title: job.title || '',
    ...
    // Fallback or browse if browse feed needed
    if (allJobs.length === 0) {
      const pagesToFetch = 10;
      for (let page = 0; page < pagesToFetch; page++) {
        const offset = page * 20;
        const response = await fetchWithTimeout(
          `https://himalayas.app/jobs/api?offset=${offset}&limit=20`
        );
...
async function fetchJobicyJobs(query = '') {
  try {
    const url = query
      ? `https://jobicy.com/api/v2/remote-jobs?count=100&tag=${encodeURIComponent(query)}`
...
async function fetchWeWorkRemotelyJobs() {
  ...
  const WWR_FEEDS = [
    ...
    'https://weworkremotely.com/categories/remote-mobile-programming-jobs.rss'
  ];
...
/**
 * Ensures a job is strictly 100% fully remote by filtering out hybrid/onsite/office keywords
 */
function isFullyRemoteJob(job) {
  ...
  const nonRemotePatterns = [
    ...
    /\bin person\b/,
    /\boffice days?\b/,
    /\bdays? in (the )?office\b/,
    /\brelocation (is )?required\b/,
    /\bmust be located near office\b/,
    /\bpartially remote\b/,
    /\bno remote\b/
  ];

  for (const pattern of nonRemotePatterns) {
    if (pattern.test(text)) {
      return false; // Reject hybrid/onsite role
    }
  }
...
async function searchJobs(resumeData) {
  ...
  // Build list of additional Remotive category queries for broad coverage
  const remotiveCategories = ['software-dev', 'devops-sysadmin', 'data', 'product'];

  // Fetch live from all major remote APIs/RSS feeds in parallel — Wave 1
  let [remotiveJobs, remoteOKJobs, himalayasJobs, arbeitnowJobs, jobicyJobs, wwrJobs, jobspressoJobs, workingNomadsJobs, nodeskJobs] = await Promise.all([
  ...
  // Wave 2: Full-feed + secondary query expansion for maximum coverage
  const wave2Promises = [
    fetchRemotiveJobs(''),           // Full Remotive feed
    fetchHimalayasJobs(secondaryQuery || ''),
    fetchJobicyJobs(secondaryQuery || '')
  ];

  // Also fetch additional Remotive category feeds if no specific query
  if (!primaryQuery) {
    for (const cat of remotiveCategories) {
      wave2Promises.push(
        fetchWithTimeout(`https://remotive.com/api/remote-jobs?category=${cat}&limit=100`)
  ...
  const [extraRemotive, extraHimalayas, extraJobicy, ...extraCategoryResults] = wave2Results.map(r => r.status === 'fulfilled' ? r.value : []);

  remotiveJobs  = [...remotiveJobs,  ...(extraRemotive  || []), ...extraCategoryResults.flat()];
  himalayasJobs = [...himalayasJobs, ...(extraHimalayas || [])];
  jobicyJobs    = [...jobicyJobs,    ...(extraJobicy    || [])];
  ...
  console.log(`📡 Fetched: Remotive(${sourceStats.remotive}), RemoteOK(${sourceStats.remoteOK}), Himalayas(${sourceStats.himalayas}), Arbeitnow(${sourceStats.arbeitnow}), Jobicy(${sourceStats.jobicy}), WWR(${sourceStats.weWorkRemotely}), Jobspresso(${sourceStats.jobspresso}), WorkingNomads(${sourceStats.workingNomads}), NoDesk(${sourceStats.nodesk})`);

  // Combine all jobs from all major remote platforms
  let allJobs = [...remotiveJobs, ...remoteOKJobs, ...himalayasJobs, ...arbeitnowJobs, ...jobicyJobs, ...wwrJobs, ...jobspressoJobs, ...workingNomadsJobs, ...nodeskJobs];
  ...
  // Prepare clean response (truncate descriptions for payload size)
  const cleanJob = (job) => ({
    id: job.id,
    title: job.title,
    company: job.company,
    ...
    jobType: Array.isArray(job.jobType) ? String(job.jobType[0] || '') : String(job.jobType || ''),
